...
const TRANSLATIONS = {
    pl: {
        ...
        err_url_fetch_failed: "Nie udało się pobrać treści tej strony — to może być spowodowane blokadą po stronie tego serwisu (np. jego ochroną antybotową), a nie problemem po naszej stronie. Sprawdź, czy link jest poprawny i publicznie dostępny. Możesz też skopiować treść artykułu i wkleić ją w trybie „Tekst” — zadziała niezależnie od tego problemu.",
    ...
    en: {
        ...
        err_url_fetch_failed: "We couldn't fetch this page — this can happen because the source site itself blocks automated access (e.g. anti-bot protection), not necessarily a problem on our end. Check that the link is correct and publicly accessible. You can also copy the article text and paste it in \"Text\" mode — that works regardless of this issue.",
    ...
    es: {
        ...
        err_url_fetch_failed: "No pudimos obtener el contenido de esta página — esto puede deberse a que el propio sitio bloquea el acceso automático (por ejemplo, protección antibots), no necesariamente un problema de nuestro lado. Comprueba que el enlace sea correcto y esté disponible públicamente. También puedes copiar el texto del artículo y pegarlo en el modo «Texto» — funciona independientemente de este problema.",
    ...
    de: {
        ...
        err_url_fetch_failed: "Wir konnten den Inhalt dieser Seite nicht abrufen — das kann daran liegen, dass die Seite selbst automatisierte Zugriffe blockiert (z. B. durch einen Anti-Bot-Schutz), nicht unbedingt an einem Problem auf unserer Seite. Prüfe, ob der Link korrekt und öffentlich zugänglich ist. Du kannst den Artikeltext auch kopieren und im Modus „Text“ einfügen — das funktioniert unabhängig von diesem Problem.",
    ...
    fr: {
        ...
        err_url_fetch_failed: "Impossible de récupérer le contenu de cette page — cela peut venir du site lui-même, qui bloque les accès automatisés (par exemple une protection anti-robots), et non forcément d'un problème de notre côté. Vérifiez que le lien est correct et accessible publiquement. Vous pouvez aussi copier le texte de l'article et le coller en mode « Texte » — cela fonctionne indépendamment de ce problème.",
    ...
    ru: {
        ...
        err_url_fetch_failed: "Не удалось получить содержимое этой страницы — возможно, сам сайт блокирует автоматический доступ (например, защита от ботов), а не проблема на нашей стороне. Проверьте, что ссылка верна и общедоступна. Вы также можете скопировать текст статьи и вставить его в режиме «Текст» — это сработает независимо от этой проблемы.",
    ...
    zh: {
        ...
        err_url_fetch_failed: "无法获取该页面的内容——这可能是因为该网站本身阻止了自动访问(例如反爬虫防护),不一定是我们这边的问题。请检查链接是否正确且可公开访问。你也可以复制文章文本，粘贴到「文本」模式中——这不受此问题影响。",
    ...
    ja: {
        ...
        err_url_fetch_failed: "このページの内容を取得できませんでした — これはサイト側が自動アクセスをブロックしている(例:ボット対策)ために起こることがあり、必ずしもこちら側の問題とは限りません。リンクが正しく、一般公開されているか確認してください。記事のテキストをコピーして「テキスト」モードに貼り付けることもできます — この問題とは関係なく動作します。",
    ...
    hi: {
        ...
        err_url_fetch_failed: "हम इस पेज की सामग्री प्राप्त नहीं कर सके — ऐसा इसलिए हो सकता है क्योंकि यह साइट खुद स्वचालित पहुंच को रोकती है (जैसे बॉट-रोधी सुरक्षा), जरूरी नहीं कि यह हमारी तरफ की समस्या हो। जांचें कि लिंक सही है और सार्वजनिक रूप से उपलब्ध है। आप लेख का टेक्स्ट कॉपी करके “टेक्स्ट” मोड में पेस्ट भी कर सकते हैं — यह इस समस्या के बावजूद काम करेगा।",
    ...
    ar: {
        ...
        err_url_fetch_failed: "تعذّر جلب محتوى هذه الصفحة — قد يكون ذلك بسبب حظر الموقع نفسه للوصول الآلي (مثل الحماية من الروبوتات)، وليس بالضرورة مشكلة من جانبنا. تحقق من أن الرابط صحيح ومتاح للجميع. يمكنك أيضًا نسخ نص المقال ولصقه في وضع «نص» — سيعمل ذلك بغض النظر عن هذه المشكلة.",
