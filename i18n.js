...
const TRANSLATIONS = {
    ...
    es: {
        ...
        err_url_fetch_failed: "No pudimos obtener el contenido de esta página — este tipo de problema ocurre cuando el propio sitio bloquea el acceso automático (por ejemplo, protección antibots). Comprueba que el enlace sea correcto y esté disponible públicamente. También puedes copiar el texto del artículo y pegarlo en el modo «Texto» — funciona independientemente de este problema.",
    ...
    de: {
        ...
        err_url_fetch_failed: "Wir konnten den Inhalt dieser Seite nicht abrufen — solche Probleme treten auf, wenn die Seite selbst automatisierte Zugriffe blockiert (z. B. durch einen Anti-Bot-Schutz). Prüfe, ob der Link korrekt und öffentlich zugänglich ist. Du kannst den Artikeltext auch kopieren und im Modus „Text“ einfügen — das funktioniert unabhängig von diesem Problem.",
    ...
    ru: {
        ...
        err_url_fetch_failed: "Не удалось получить содержимое этой страницы — такие проблемы случаются, когда сам сайт блокирует автоматический доступ (например, защита от ботов). Проверьте, что ссылка верна и общедоступна. Вы также можете скопировать текст статьи и вставить его в режиме «Текст» — это сработает независимо от этой проблемы.",
    ...
    zh: {
        ...
        err_url_fetch_failed: "无法获取该页面的内容——当该网站本身阻止自动访问时(例如反爬虫防护)就会出现这类问题。请检查链接是否正确且可公开访问。你也可以复制文章文本，粘贴到「文本」模式中——这不受此问题影响。",
    ...
    ja: {
        ...
        err_url_fetch_failed: "このページの内容を取得できませんでした — このような問題は、サイト側が自動アクセスをブロックしている場合(例:ボット対策)に起こります。リンクが正しく、一般公開されているか確認してください。記事のテキストをコピーして「テキスト」モードに貼り付けることもできます — この問題とは関係なく動作します。",
    ...
    ar: {
        ...
        err_url_fetch_failed: "تعذّر جلب محتوى هذه الصفحة — تحدث مثل هذه المشكلات عندما يحظر الموقع نفسه الوصول الآلي (مثل الحماية من الروبوتات). تحقق من أن الرابط صحيح ومتاح للجميع. يمكنك أيضًا نسخ نص المقال ولصقه في وضع «نص» — سيعمل ذلك بغض النظر عن هذه المشكلة.",
