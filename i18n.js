...
const TRANSLATIONS = {
    pl: {
        ...
        err_url_fetch_failed: "Nie udało się pobrać treści tej strony — problemy tego typu zdarzają się, kiedy mamy do czynienia z blokadą po stronie serwisu (np. ochrona antybotowa). Możesz też skopiować treść artykułu, wkleić ją w trybie „Tekst” i podać tam ten sam link w polu „Opcjonalnie: link do źródła” — zadziała niezależnie od tego problemu, a tym samym pomożesz też innym, którzy wkleją ten sam link.",
    ...
    es: {
        ...
        err_url_fetch_failed: "No pudimos obtener el contenido de esta página — este tipo de problema ocurre cuando el propio sitio bloquea el acceso automático (por ejemplo, protección antibots). También puedes copiar el texto del artículo, pegarlo en el modo «Texto» y añadir ese mismo enlace en el campo «Opcional: enlace de la fuente» — funciona independientemente de este problema, y así también ayudas a otras personas que peguen el mismo enlace.",
    ...
    de: {
        ...
        err_url_fetch_failed: "Wir konnten den Inhalt dieser Seite nicht abrufen — solche Probleme treten auf, wenn die Seite selbst automatisierte Zugriffe blockiert (z. B. durch einen Anti-Bot-Schutz). Du kannst den Artikeltext auch kopieren, ihn im Modus „Text“ einfügen und denselben Link im Feld „Optional: Quellenlink“ angeben — das funktioniert unabhängig von diesem Problem und hilft damit auch anderen, die denselben Link einfügen.",
    ...
    ru: {
        ...
        err_url_fetch_failed: "Не удалось получить содержимое этой страницы — такие проблемы случаются, когда сам сайт блокирует автоматический доступ (например, защита от ботов). Вы также можете скопировать текст статьи, вставить его в режиме «Текст» и указать ту же ссылку в поле «Необязательно: ссылка на источник» — это сработает независимо от этой проблемы, а заодно поможет другим, кто вставит ту же ссылку.",
    ...
    zh: {
        ...
        err_url_fetch_failed: "无法获取该页面的内容——当该网站本身阻止自动访问时(例如反爬虫防护)就会出现这类问题。你也可以复制文章文本，粘贴到「文本」模式中，并在「可选:来源链接」字段中填入同一个链接——这不受此问题影响，还能帮助其他粘贴同一链接的人。",
    ...
    ja: {
        ...
        err_url_fetch_failed: "このページの内容を取得できませんでした — このような問題は、サイト側が自動アクセスをブロックしている場合(例:ボット対策)に起こります。記事のテキストをコピーして「テキスト」モードに貼り付け、「任意:出典リンク」欄に同じリンクを入力することもできます — この問題とは関係なく動作するうえ、同じリンクを貼る他の人の助けにもなります。",
    ...
    hi: {
        ...
        err_url_fetch_failed: "हम इस पेज की सामग्री प्राप्त नहीं कर सके — ऐसी समस्याएं तब होती हैं जब कोई साइट खुद स्वचालित पहुंच को रोकती है (जैसे बॉट-रोधी सुरक्षा)। आप लेख का टेक्स्ट कॉपी करके “टेक्स्ट” मोड में पेस्ट कर सकते हैं और वही लिंक “वैकल्पिक: स्रोत लिंक” फ़ील्ड में भी डाल सकते हैं — यह इस समस्या के बावजूद काम करेगा, और उसी लिंक को पेस्ट करने वाले दूसरों की भी मदद करेगा।",
    ...
    ar: {
        ...
        err_url_fetch_failed: "تعذّر جلب محتوى هذه الصفحة — تحدث مثل هذه المشكلات عندما يحظر الموقع نفسه الوصول الآلي (مثل الحماية من الروبوتات). يمكنك أيضًا نسخ نص المقال، ولصقه في وضع «نص»، وإضافة نفس الرابط في حقل «اختياري: رابط المصدر» — سيعمل ذلك بغض النظر عن هذه المشكلة، وسيساعد أيضًا الآخرين الذين سيلصقون نفس الرابط.",
