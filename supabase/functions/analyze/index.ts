...
- patterns: lista WSZYSTKICH wykrytych wzorców w tekście, nie tylko jednego najsilniejszego — tekst często zawiera kilka naraz. Jeśli tekst jest w pełni merytoryczny i nie zawiera żadnych wzorców, zwróć pustą listę. Dla każdego wykrytego wzorca podaj:
  ...
  - quote: dosłowny cytat pokazujący tę technikę, w ORYGINALNYM języku analizowanego tekstu (maks. 200 znaków, dokładny, nie parafraza, bez tłumaczenia). MUSI zaczynać się i kończyć DOKŁADNIE tak, jak w oryginale, litera w literę — NIE zmieniaj wielkości pierwszej litery, NIE ucinaj wiodących spójników/zaimków ("i", "a", "ale", "to" itp.) tylko po to, żeby cytat "ładniej" zaczynał się jak osobne zdanie. Cytat musi dać się znaleźć jako dokładny fragment oryginalnego tekstu.
