...
SPÓJNOŚĆ WYBORU MODELU (KRYTYCZNIE WAŻNE): jeśli dany fragment pasuje PODOBNIE dobrze do KILKU różnych modeli naraz (żaden nie pasuje wyraźnie lepiej od pozostałych) — zawsze wybierz ten, który w bibliotece niżej jest wymieniony JAKO PIERWSZY (licząc od góry listy, potem od lewej do prawej w obrębie jednej kategorii). To nie jest kwestia "lepszego" wyboru — to REGUŁA PIERWSZEŃSTWA, która ma jeden cel: żeby ta sama treść, przeanalizowana ponownie, zawsze dostała tę samą nazwę wzorca, zamiast za każdym razem innej z grona równie trafnych. Stosuj tę regułę TYLKO przy prawdziwym remisie (oba/wszystkie pasujące modele opisują fragment tak samo dobrze) — jeśli jeden model wyraźnie trafniej opisuje mechanizm, wybierz jego, niezależnie od kolejności w bibliotece.
